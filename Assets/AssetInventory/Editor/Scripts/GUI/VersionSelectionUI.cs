        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (_packageInfo == null)
            {
                if (_info.PackageSource == PackageSource.Git)
                {
                    ...
                    if (GUILayout.Button(UIStyles.Content($"Install Indexed {_info.GetVersion()}")))
                    {
                ...
                else
                {
                    ...
                    {
                        if (GUILayout.Button(UIStyles.Content($"Install Indexed {_info.LatestVersion}")))
                        {
                            _callback?.Invoke(_info.LatestVersion);
                            editorWindow.Close();
                        }
            ...
            foreach (string version in _packageInfo.versions.all.Reverse())
            {
                ...
                if (GUILayout.Button(UIStyles.Content(version, "Install this version"), GUILayout.Width(150)))
                {
